...
class Mage_Review_Model_Resource_Review_Collection extends Mage_Core_Model_Resource_Db_Collection_Abstract
{
    ...
    protected function _initSelect()
    {
        ...
        $this->getSelect()
            ->join(array('detail' => $this->_reviewDetailTable),
                ...
                array('detail_id', 'title', 'detail', 'nickname', 'customer_id','store_id'));
        return $this;
    }
    ...
    public function addCustomRateVotes()
    {
        foreach ($this->getItems() as $item) {
            $votesCollection = Mage::getModel('rating/rating_option_vote')
                ->getResourceCollection()
                ->setReviewFilter($item->getId())
                ->setStoreFilter($item->getStoreId())
                ->addRatingInfo($item->getStoreId())
                ->load();
            $item->setCustomRatingVotes($votesCollection);
        }
